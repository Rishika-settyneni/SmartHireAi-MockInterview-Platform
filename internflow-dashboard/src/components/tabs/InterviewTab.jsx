...
const InterviewTab = ({ 
  ...
  const handleSubmitWithRecording = async () => {
    ...
    try {
      await handleSubmitInterviewOriginal();
    } catch (error) {
      console.error('Error submitting interview:', error);
    }

    console.log('🎯 Starting comprehensive interview scoring...');
    if (webcamRef.current && interviewId) {
      try {
        const result = await webcamRef.current.scoreInterview();
        if (!result) {
          console.warn(
            '⚠️ Comprehensive scoring did not complete — History/Progress will show an ' +
            'estimated Communication/Confidence/Technical Relevance/Professionalism breakdown ' +
            'derived from the legacy score instead of the ML-scored one.'
          );
        } else {
